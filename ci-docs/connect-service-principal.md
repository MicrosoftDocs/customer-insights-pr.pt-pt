...
O Dynamics 365 Customer Insights fornece a opção de se ligar a uma conta do Azure Data Lake Storage utilizando um principal de serviço do Azure, em vez de chaves de conta de armazenamento.

As ferramentas automatizadas que utilizam os serviços Azure devem ter permissões restritas. Em vez de ter as informações de início de sessão das aplicações como um utilizador totalmente privilegiado, o Azure oferece os principais de serviço. Utilize os principais de serviço para [adicionar ou editar uma pasta do Common Data Model como uma origem de dados](connect-common-data-model.md) em segurança ou [criar ou atualizar um ambiente](create-environment.md).

## <a name="prerequisites"></a>Pré-requisitos

- A conta do Data Lake Storage que utilizará o principal de serviço tem de ser de segunda geração. As contas de armazenamento do Azure Data Lake Gen1 não são suportadas.
- A conta do Data Lake Storage tem o [espaço de nomes hierárquico ativado](/azure/storage/blobs/data-lake-storage-namespace).
- Permissões de admin para o seu inquilino do Azure, se tiver de criar um novo principal de serviço.
...
Antes de criar um novo principal de serviço para o Customer Insights, verifique se já existe na sua organização. Na maioria dos casos, já existe.
...
5. Se encontrar um registo correspondente, significa que o principal de serviço já existe. [Conceda permissões](#grant-permissions-to-the-service-principal-to-access-the-storage-account) ao principal de serviço para aceder à conta de armazenamento.
...
6. Se não surgirem resultados, [crie um novo principal de serviço](#create-a-new-service-principal).
...
Para conceder permissões ao principal do serviço para a conta de armazenamento que pretende utilizar nos Customer Insights, uma das seguintes funções tem de ser atribuída à conta ou contentor de armazenamento:

|Credencial|Requisitos|
|----------|------------|
|Utilizador com sessão iniciada atualmente|**Função**: Leitor de Dados de Blob de Armazenamento, Contribuidor de Blob de Armazenamento ou Proprietário de Blob de Armazenamento.<br>**Nível**: permissões concedidas na conta de armazenamento ou no contentor.</br>|
|Principal de Serviço do Customer Insights -<br>Utilizar o Azure Data Lake Storage como uma origem de dados</br>|Opção 1<ul><li>**Função**: Leitor de Dados de Blob de Armazenamento, Contribuidor de Dados de Blob de Armazenamento ou Proprietário de Dados de Blob de Armazenamento.</li><li>**Nível**: permissões concedidas na conta de armazenamento.</li></ul>Opção 2 *(sem partilhar o acesso do Principal de Serviço à conta de armazenamento)*<ul><li>**Função 1**: Leitor de Dados de Blob de Armazenamento, Contribuidor de Dados de Blob de Armazenamento ou Proprietário de Dados de Blob de Armazenamento.</li><li>**Nível**: permissões concedidas no contentor.</li><li>**Função 2**: Atribuidor de Dados de Blob de Armazenamento.</li><li>**Nível**: permissões concedidas na conta de armazenamento.</li></ul>|
|Principal de Serviço do Customer Insights - <br>Utilizar o Azure Data Lake Storage como uma saída ou um destino</br>|Opção 1<ul><li>**Função**: Contribuidor de Dados de Blob de Armazenamento ou Proprietário de Blob de Armazenamento.</li><li>**Nível**: permissões concedidas na conta de armazenamento.</li></ul>Opção 2 *(sem partilhar o acesso do Principal de Serviço à conta de armazenamento)*<ul><li>**Função**: Contribuidor de Dados de Blob de Armazenamento ou Proprietário de Blob de Armazenamento.</li><li>**Nível**: permissões concedidas no contentor.</li><li>**Função 2**: Atribuidor de Blob de Armazenamento.</li><li>**Nível**: permissões concedidas na conta de armazenamento.</li></ul>|
...
1. No painel **Adicionar atribuição de funções**, defina as seguintes propriedades:
   - **Função**: Leitor de dados de Blob de armazenamento, Contribuidor de blob de armazenamento ou Proprietário de blob de armazenamento com base nas credenciais listadas acima.
   - **Atribuir acesso a**: **utilizador, grupo ou principal de serviço**
   - **Selecionar** membros: **IA do Dynamics 365 para o Customer Insights** (o [principal de serviço](#create-a-new-service-principal) que procurou anteriormente neste procedimento)
...
Anexe uma conta do Data Lake Storage no Customer Insights para [armazenar dados de saída](manage-environments.md) ou [usar como origem de dados](connect-dataverse-managed-lake.md). Escolha entre uma abordagem [baseada em recursos](#resource-based-storage-account-connection) ou [baseada em subscrições](#subscription-based-storage-account-connection) e siga esses passos.
