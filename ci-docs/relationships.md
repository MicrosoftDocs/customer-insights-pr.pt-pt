...
- As relações de sistema não editáveis são criadas pelo sistema como parte do processo de unificação de dados
- As relações herdadas não editáveis são criadas automaticamente a partir da ingestão de origens de dados
- As relações personalizadas editáveis são criadas e configuradas pelos utilizadores
...
3. No painel **Nova relação**, forneça as seguintes informações:
   ...
   - **Cardinalidade fornecida com o programa**: cardinalidade da entidade de origem. Cardinalidade descreve o número de elementos possíveis num conjunto. Está sempre relacionado com a cardinalidade de destino. Pode escolher entre **Um** e **Muitos**. Só são suportadas relações muitos-para-um e um-para-um.  
   ...
   - **Cardinalidade de destino**: cardinalidade dos registos da entidade de destino.
   - **Campo de chave de origem**: campo de chave externa na entidade de origem. Exemplo: SupportCase utiliza  **CaseID** como campo de chave externa.
   - **Campo de chave de destino**: campo de chave da entidade de destino. Exemplo: o Cliente utiliza **CustomerID** como campo de chave.
...
1. No painel **Hierarquia de contas**, forneça um nome para a hierarquia. O sistema cria um nome para a entidade de saída, mas não o pode alterar.
...
1. Selecione o **UID da Conta** e o **UID Principal da Conta** da entidade selecionada.
1. Selecione **Guardar** para finalizar a hierarquia da conta.

## <a name="manage-existing-relationships"></a>Gerir relações existentes

Vá para a página **Relações** para ver todas as relações que foram criadas, a respetiva entidade de origem, a entidade de destino e a cardinalidade.
...
Utilize as opções **Filtrar por** ou **Procurar relações** para localizar uma relação específica. Para ver um diagrama de rede das relações existentes e a respetiva cardinalidade, selecione [**Visualizador**](#explore-the-relationship-visualizer).

Selecione uma relação para ver as ações disponíveis:
...
Para personalizar a vista, pode alterar a posição das caixas arrastando-as na tela. Outras opções incluem: 
